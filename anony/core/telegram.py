...
class Telegram:
    ...
    async def download(self, msg: types.Message, sent: types.Message) -> Media | None:
        ...
        if duration > config.DURATION_LIMIT:
            await sent.edit_caption(sent.lang["play_duration_limit"].format(config.DURATION_LIMIT // 60))
            return await sent.stop_propagation()

        if file_size > 200 * 1024 * 1024:
            await sent.edit_caption(sent.lang["dl_limit"])
            return await sent.stop_propagation()
        ...
        async def progress(current, total):
            ...
            text = (
                ...
                f"{utils.format_size(current)} / {utils.format_size(total)}\n"
                f"⚡ {utils.format_size(speed)}/s | ⏳ {eta}"
            )
        ...
        try:
            ...
            if not os.path.exists(file_path):
                if file_id in self.active:
                    await sent.edit_caption(sent.lang["dl_active"])
                    return await sent.stop_propagation()
